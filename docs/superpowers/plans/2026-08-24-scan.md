...
この計画のコードは、書いた後に一度リポジトリへ適用して検証してある（267件通過）。その過程で見つかった問題は本文に反映済みだが、実装時に踏みやすいので明記しておく。

1. **時計を偽装してはいけない。** `vi.useFakeTimers` は `Date.now()` を凍結するため、Testing Library の `waitFor` が経過時間を測れなくなる。さらにタイマーまで偽装すると fake-indexeddb の内部スケジューリングが止まり、トランザクションが `InvalidStateError` で落ちる。曜日に依存するテストは、実際の今日から相対的に組み立てる。
2. **記録後の再読み込みで全画面の読み込み表示に戻してはいけない。** 出したばかりの花丸と結果が一瞬で消え、先生が読めたかどうか分からなくなる。`submissions` の loading は画面全体を差し替えない。
3. **`scan-result` は空のときも存在する。** `findByTestId` で待っても中身が入る前に判定が進む。中身のテキストを待つこと。
4. **配列リテラルは `as const` を付けないと `string` に広がる。** `objectStoreNames.contains()` の引数型に合わずビルドが落ちる。
...
まず既存テストを1件だけ直す。`src/db/migration.test.ts` の「バージョンが2である」は `expect(DB_VERSION).toBe(2)` と番号を固定しており、バージョンを上げるたびに壊れる。このテストが守りたいのは「v2のストアが揃うこと」であって番号ではない。
...
`describe` の名前も「まっさらな端末でのバージョン2」から「まっさらな端末」に、`it` の名前も上記に変える。

Run: `npm run test:run && npm run build`
Expected: 224件すべてPASS、型エラーなし。

**この1件を除いて、既存テストが落ちたら移行が壊れている。** 先に進まず報告すること。
