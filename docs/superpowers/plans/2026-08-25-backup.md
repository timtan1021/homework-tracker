...
describe("parseBackup", () => {
  ...
  it("formatVersion が未知の値なら拒否する", () => {
    expect(() =>
      parseBackup(JSON.stringify({ formatVersion: 99, cohort: {} })),
...
describe("restoreBackup", () => {
  ...
  it("現在のデータを完全に置き換える", async () => {
    ...
    // 復元用のバックアップは別クラスのデータとして用意する
    const backupSource = await createCohort({ year: 2025, className: "4年1組" });
    await addStudent({ cohortId: backupSource.id, attendanceNumber: 99 });
    const backup = await buildBackup(backupSource.id);

    // 復元直前の有効クラスは "5年1組"（4年1組を作った時点で無効化されている）
    // ため、この時点で有効なのは 4年1組。5年1組のデータは無効クラスとして残る。
    // 「現在のデータ」＝有効なcohortのデータを消すことを確認する。
    await restoreBackup(backup);
    ...
    expect(students.map((s) => s.attendanceNumber)).toEqual([99]);
  });
...
describe("復元", () => {
  it("ファイルを選ぶと確認ダイアログが出る", async () => {
    ...
    const cohort = await createCohort({ year: 2025, className: "4年1組" });
    // すでに beforeEach で作られている 5年1組 を上書きしないよう、
    // ここでは別のバックアップ内容を用意する。
    ...
    const input = screen.getByLabelText("復元するファイル");
    ...
    expect(
      await screen.findByText(/現在のデータ.*を消して/),
    ).toBeInTheDocument();
  });
  ...
  it("確認すると復元される", async () => {
    const user = userEvent.setup();
    const cohort = await createCohort({ year: 2025, className: "4年1組" });
    await addStudent({ cohortId: cohort.id, attendanceNumber: 7 });
    const backup = await buildBackup(cohort.id);
  ...
  it("取り消せば何も変わらない", async () => {
    const user = userEvent.setup();
    const other = await createCohort({ year: 2025, className: "4年1組" });
    const backup = await buildBackup(other.id);
...
**この追記に必要な import を、ファイル先頭の既存import群に足すこと。**

```tsx
import { beforeEach } from "vitest"; // 既存の import 群に describe/it/expect と並んで無ければ足す
import { createCohort, getActiveCohort } from "../db/cohorts";
import { addStudent } from "../db/students";
import { buildBackup } from "../backup/export";
```

既存のテストファイルの先頭に `beforeEach` で `5年1組` のcohortを作る処理がすでにあるかを確認し、無ければ次を追加すること（他の `describe` と同じ階層、ファイル冒頭）。
...
既存のテストと重複するセットアップがあれば、二重に `beforeEach` を書かず1つにまとめること。
...
`import type { BackupFile } from "../backup/types";` をこのテストファイル冒頭のimportに足すこと。
